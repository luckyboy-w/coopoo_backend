  ...
  methods: {
    deleteRow(rowIndex, data) {
      ...
      this.$confirm("是否继续删除操作?", "提示", {
        ...
        postMethod("/backend/coupon/delete", param).then(res => {
          this.loadList();
          this.$message("删除成功");
        });
      });
    ...
    addOrEdit(oper, rowIndex, data) {
      ...
      if (oper == "edit") {
        ...
        getMethod("/backend/coupon/findObject", param).then(res => {
          scope.editData = res.data[0];
          this.showList = false;
          this.showAddOrEdit = true;
        });
    ...
    loadList() {
      let scope = this;
      getMethod("/backend/coupon/findPage?isGift=1", this.searchParam).then(res => {
