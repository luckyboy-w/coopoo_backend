...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // exportApplyCash_() {
      //   let param = [];
      //   for (let key in this.exportApplyFrm_) {
      //     if (null != this.exportApplyFrm_[key]) {
      //       param.push(key + '=' + this.exportApplyFrm_[key])
      //     }
      //   }
      //   //window.open(process.env.VUE_APP_BASE_API+'/backend/siteData/selectCashProcessExport?'+param.join('&'))
      ...
      //     if (null != this.exportApplyFrm_[key]) {
      //       param.push(key + '=' + this.exportApplyFrm_[key])
      //       form.append($('<input/>').attr('type', 'hidden').attr('name', key).attr('value', this.exportApplyFrm_[key]));
      //     }
      //   }
      //   form.appendTo('body').submit().remove();
      // },
