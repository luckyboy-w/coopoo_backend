<template>
    <div style="padding-top:20px;width:100%">
         ...
         <el-row style="line-height:40px;padding:0px 25px 5px 25px">
            <el-col :span="3" style="font-size:14px;">平台运营至今</el-col>
            <el-col :span="2" style="font-size:14px;"></el-col>
        ...
        <el-tabs v-model="activeName" type="border-card" @tab-click="handleClick" >
            <el-tab-pane label="返佣数据" name="allFee" style="height:600px">
                <el-row style="line-height:40px;padding:10px 0px ">
                    <el-col :span="24" style="padding-left:10px">
                        <el-button type="primary" v-if="!allFeeDataShow" @click="backToAllFee()" icon="el-icon-back">返回列表</el-button>
                    </el-col>
                </el-row>
                <el-table
                    v-if="allFeeDataShow"
                    ...
                    row-key="id">
                    <el-table-column prop="provinceName" label="服务商" min-width="20%"></el-table-column>
                    <el-table-column prop="mobileNo" label="联系电话" min-width="20%"></el-table-column>
                    <el-table-column prop="cashNum" label="返佣金额" min-width="20%">
                    ...
                    <el-table-column prop="provId" label="操作" min-width="20%">
                        <template slot-scope="scope">
                            <el-link type="primary" @click="chkFeeDtl(scope.row)" >查看明细</el-link>
                            <el-link type="primary" @click="chkFriDtl(scope.row)" >会员明细</el-link>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
                ...
                <el-table
                   ...
                   <el-table-column prop="friName" label="会员昵称" min-width="24%">
                    ...
                    <el-table-column prop="tradeTime" label="消费时间" min-width="24%">
                             <template slot-scope="scope">
            ...
            <el-tab-pane label="提现中" name="feeProcess" style="height:600px">
                <el-row style="line-height:40px;padding:10px 0px ">
                    <el-col :span="1" style="font-size:14px;">申请单号</el-col>
                    <el-col :span="3">
                         <el-input  v-model="exportApplyFrm.cashNo" style="width:80px" placeholder=""></el-input>
                    </el-col>
                    <el-col :span="1" style="font-size:14px;">服务商</el-col>
                    <el-col :span="3">
                        <el-select v-model="exportApplyFrm.provId" placeholder="请选择服务商">
                            <el-option
                    ...
                    <el-col :span="9" style="padding-left:10px">
                        <el-button type="primary" @click="batchBill()">搜索</el-button>
                        <el-button type="primary" @click="exportApplyCash()" plain icon="el-icon-download">导出</el-button>
                    </el-col>
                </el-row>
                <el-table
                    ref="feeProcessData"
                    ...
                    <el-table-column prop="cashNum" label="结算金额（元）" min-width="24%">
                        <template slot-scope="scope">
                            {{scope.row.cashNum | fmtFee}}
                        </template>
                    </el-table-column>
                    <el-table-column prop="pkBillId" label="操作" min-width="24%">
                        <template slot-scope="scope">
                            <el-link type="primary" @click="dealCash(scope.row)" >完成提现</el-link>
    ...
    methods: {
        loadPlatFee(){
            ...
            getMethod('/backend/siteData/selectPlatFee', {}).then(res => {
                let platData =  res.data[0]
                scope.billFeeText = platData.billFee
                scope.allApplyFee = platData.allApplyFee
                scope.allPerFee = platData.allPerFee
            })
        },
        exportApplyCash(){
            let param = [];
            ...
            var form = $('<form></form>').attr('action',process.env.VUE_APP_BASE_API+'/backend/siteData/selectCashProcessExport').attr('method','post');
            for(let key in this.exportApplyFrm){
                if(null != this.exportApplyFrm[key]){
                    param.push(key+'='+this.exportApplyFrm[key])
