<template>
  <div>
          ...
          <tr>
            ...
            <td>
              <el-select v-model="searchParam.tenantId" placeholder="请选择">
                ...
                <el-option v-for="item in supplyList" :key="item.id" :value-key="item.supplierName" :label="item.supplierName"
                  :value="item.id" />
              </el-select>
            </td>
            <!-- <td>风格状态:</td>
            ...
            <el-table-column prop="id" label="操作" width="200px">
              <template slot-scope="scope">
                <div v-if="scope.row.verifyStatus != 30">
                  <el-button v-if="scope.row.verifyStatus == 10 " type="text" size="small" @click.native.prevent="verifyGood(scope.row)">
                    审核
                  </el-button>
                  <!--审核通过才能操作上架下架-->
    ...
    <saveOrEdit v-if="showAddOrEdit" :is-gift="isGift" :edit-data="editData" :is-edit-good="isEditGood" @showListPanel="showListPanel" />
  </div>
</template>
