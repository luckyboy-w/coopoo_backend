<template>
	<div>
		<div class="ly-container" v-if="!showAddOrEdit" v-loading="isLoading">
                        ...
                        <el-table-column type="expand">
                            <template slot-scope="props">
                                <el-table
									...
									<el-table-column prop="supplyPrice" label="供货价" width="150px" ></el-table-column>
						...
						<el-table-column prop="id" label="操作" width="200px">
							<template slot-scope="scope">
									<div v-if="scope.row.verifyStatus != 30">
										<el-button v-if="scope.row.verifyStatus == 10 " type="text" size="small" @click.native.prevent="verifyGood(scope.row)">审核</el-button>
										<!--审核通过才能操作上架下架-->
										<!--审核状态;10:待审核;20:已通过;30:被驳回-->
										<!--5:在仓库;1:上架;2:下架;3:违规下架-->
										<el-button v-if="scope.row.verifyStatus == 20 && scope.row.isSale == 2"  type="text" size="small" @click.native.prevent="onLine(scope.row)">上架</el-button>
										<el-button v-if="scope.row.verifyStatus == 20 && scope.row.isSale == 2"   type="text" size="small" @click.native.prevent="editGood(scope.row)">编辑</el-button>
										<el-button v-if="scope.row.verifyStatus == 20 && scope.row.isSale == 1" type="text" size="small" @click.native.prevent="offLine(scope.row,'3')">违规下架</el-button>
										<el-button v-if="scope.row.verifyStatus == 20 && scope.row.isSale == 1" type="text" size="small" @click.native.prevent="offLine(scope.row,'2')">下架</el-button>
									</div>
							</template>
						</el-table-column>
					</el-table>
		...
		<saveOrEdit v-if="showAddOrEdit" @showListPanel="showListPanel" :isGift="isGift" :editData="editData" :isEditGood="isEditGood" ></saveOrEdit>
	</div>
</template>
