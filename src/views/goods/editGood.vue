      ...
      <el-col :xs="24" :sm="24" :lg="24" class="card-panel-col">
        <el-form ref="dataForm" :model="dataForm" label-width="100px" label-position="left">
          <el-form-item label="商品名称">
            <el-input v-model="dataForm.goodsName" style="width:260px" placeholder="请输入商品名称" :disabled="isDisabled" maxlength="30" show-word-limit />
          </el-form-item>
          <el-form-item label="商品卖点">
            <el-input v-model="dataForm.sellingPoint" style="width:260px" placeholder="请输入卖点" maxlength="50" :disabled="isDisabled" show-word-limit />
          </el-form-item>
          ...
          <el-form-item label="属性选择">
              ...
              <el-table ref="dragTable" :data="dbAttrList" style=" margin-bottom: 20px;" row-key="onlyKey" :show-header="false">
                <el-table-column>
                  <template slot-scope="scope">
                    ...
                    <div class="attr-save">
                      <el-input style="width:200px;margin-right: 30px" :disabled="isDisabled" placeholder="请输入自定义值" v-model="scope.row.newAttrValue" />
                      <el-button type="primary" @click="handlerAttrData(scope.row)" :disabled="isDisabled">添加</el-button>
            ...
            <template>
              ...
              <div class="attr-save"><el-button @click="addAttrNameInput" type="primary" :disabled="isDisabled">添加</el-button></div>
              ...
              <template v-for="(attrItem, index) in addAttrParam">
                ...
                <div class="attr-content">
                  ...
                  >
                    <el-button
                      :disabled="attrItem.skuList.length === 1 || isDisabled"
          ...
          <el-form-item label="新SKU配置">
            <el-table style="width: 100%; margin-bottom: 20px;" :data="tableList" :span-method="objectSpanMethod" border>
              ...
              <el-table-column align="center" prop="goodsImgUrl" label="SKU展示图">
                <template slot-scope="scope">
                  <img :src="scope.row.goodsImgUrl" width="60px" height="60px" onerror="this.src='https://bluemobi-lanyu.oss-cn-shanghai.aliyuncs.com/static/black_bg.png' " />
                </template>
              </el-table-column>
          ...
          <el-form-item label="视频">
            ...
            <el-upload
              :class="{ hide: hideGoodVideoUpload }"
            ...
            >
              <video
                v-if="dataForm.goodsVideo != '' && !videoFlag"
                v-bind:src="goodsVideoUrl"
                class="video-avatar"
              ...
              <div slot="tip" class="el-upload__tip">推荐视频尺寸：1:1或16:9</div>
            </el-upload>
            <el-dialog><img width="100%" :src="imageUrl" alt /></el-dialog>
          </el-form-item>
          ...
          <el-form-item label="封面图片">
            <div id="front-img">
              ...
              <el-upload
                :action="uploadGoodsCoverImageUrl"
                list-type="picture-card"
                :disabled="isDisabled"
                :on-preview="handleGoodImagePreview"
                :before-upload="beforeGoodsCoverImageUpload"
                :on-success="handleGoodsCoverImageSuccess"
                :class="{ hide: hideGoodsCoverImageUpload }"
                :file-list="uploadGoodsCoverImageList"
                :on-remove="handleGoodsCoverImageRemove"
              >
                <i :class="addFrontCls" />
          ...
          <el-form-item label="售后说明图片">
            <el-radio-group v-model="dataForm.postSaleId">
              <el-radio class="my-el-radio" :disabled="isDisabled" v-for="(item, index) in goodSaleDescList" :key="item.id" :label="item.id">
                <div class="my-container">
                  <div class="img" @mouseover="changeImgMask(index, true)" @mouseout="changeImgMask(index, false)">
                    <div ref="imgMask" class="img-mask"><i class="el-icon-zoom-in my-icon" @click="showBigImg(item.imgUrl)"></i></div>
          ...
          <el-form-item label="商品详情"><qEditor ref="refEditor" :content="detail.detailContent" module-name="detailContent" @changeContent="changeContent" /></el-form-item>
        </el-form>
      </el-col>
    </el-row>

    <el-dialog center :visible.sync="goodSaleDescImgVisible"><img width="100%" :src="goodSaleDescImgUrl" alt /></el-dialog>
...
export default {
  ...
  methods: {
    ...
    loadSupplierList() {
      let scope = this;
      getMethod('/supplier/search-supplier-list', { pageSize: 50, pageNum: 1 }).then(res => {
    ...
    handleGoodImageRemove(res) {
      for (let i = 0; i < this.uploadGoodImageList.length; i++) {
        if (this.uploadGoodImageList[i].filePath == (res.filePath || res.response.data.filePath)) {
    ...
    handleGoodsCoverImageRemove(res) {
      for (let i = 0; i < this.uploadGoodsCoverImageList.length; i++) {
        if (this.uploadGoodsCoverImageList[i].filePath == (res.filePath || res.response.data.filePath)) {
    ...
    initDefaultImage(data) {
      ...
      (this.uploadGoodsCoverImageList = data.goodsCoverImg ? data.goodsCoverImg : []), (this.goodsVideoUrl = data.goodsVideo ? data.goodsVideo[0].url : '');
      this.uploadVideoList = data.goodsVideo ? data.goodsVideo : [];
      if (this.uploadGoodsCoverImageList.length >= 1) {
        document.getElementById('front-img').getElementsByClassName('el-upload--picture-card')[0].style.display = 'none';
      }
    },
    isMoney(amount, type, errorMsg) {
      // 22,111,22.11   判断是否是金额
      if (errorMsg != '') {
        return errorMsg;
      }
      if (amount == '0.0' || amount == '0.00' || amount == '0.') {
        return type + '的金额格式有误';
      }
    ...
    async saveObject() {
      if (this.validate()) {
        ...
        for (let i = 0; i < this.tableList.length; i++) {
          ...
          if (rowObj.goodsImg == '' || rowObj.goodsImg == undefined) {
            errorMsg = '请上传sku展示图';
          }
          if (rowObj.salePrice == '') {
            errorMsg = '会员价不能为空';
        ...
        for (let i = 0; i < this.dbAttrList.length; i++) {
          ...
          for (let j = 0; j < dbSkuList.length; j++) {
            skuTextArr.push(dbSkuList[j].skuText);
            skuTextArr_ = [...new Set(skuTextArr)];
          }
          if (skuTextArr.length != skuTextArr_.length) {
            this.$message({
              message: '商品属性规格值不能重复',
              type: 'warning'
            });
        ...
        textList.map((item, index) => {
          if (item.specificationValueParamList.length == 0) {
    ...
    addColumn(dataList, specName, specValue, type) {
      ...
      for (let i = 0; i < newDataList.length; i++) {
        ...
        if (this.oldTableMap.get(key)) {
          ...
          // goodsImgUrl: ''
          // 用老的数据替换新的这几个值
          let { stock, marketPrice, salePrice, goodsCode, supplyPrice, goodsImgUrl, skuId, goodsImg } = oldData;
          newDataList[i] = {
            ...newDataList[i],
            stock,
            marketPrice,
      ...
      //     if (JSON.stringify(newDataList[i])==JSON.stringify(newDataList[j])) {
    ...
    checkSpecValueUnique(attrSkuList) {
      ...
      var isDuplicate = skuTextArray.some((item, idx) => skuTextArray.indexOf(item) != idx);
      if (isDuplicate) {
    ...
    checkSpecNameUnique(attrList) {
      ...
      var isDuplicate = attrTextArray.some((item, idx) => attrTextArray.indexOf(item) != idx);
      if (isDuplicate) {
