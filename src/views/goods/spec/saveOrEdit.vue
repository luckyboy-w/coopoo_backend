...
export default {
  ...
  methods: {
    ...
    findGoodType(){
        getMethod('/backend/goodType/findList', this.searchParam).then(res => {
    ...
    saveObject(){
        ...
        if(this.validate()){
            ...
            let param = {
                jsonSpec:JSON.stringify(this.dataForm)
            }
            postMethod("/backend/goodSpec/update", param).then(res => {
    ...
    submitUpdate() {
        ...
        // getMethod("/backend/goodSpec/findObj", param).then(res => {
        //     if(res.data.length != 0 ){
        //         this.$message({
        //             message: '规则名称有重复，请重新录入，如徐新增属性，请修改对应的规则',
        //             type: 'warning'
        //         });
        //     }else{
        //         this.saveObject()
