			...
			<div class="ly-table-panel">
				<div class="ly-data-list">
					...
					>
						...
						<el-table-column prop="attrValue" label="属性值" width="750px"></el-table-column>
...
export default {
	...
	methods: {
		...
		batchDeleteRow(rowIndex, data) {
			...
			postMethod("/backend/goodAttr/delete", param).then(res => {
		...
		addOrEdit(oper, rowIndex, data) {
			...
			if (oper == "edit") {
				...
				getMethod("/backend/goodAttr/findList", param).then(res => {
					scope.editData = res.data[0];
					this.showList = false;
					this.showAddOrEdit = true;
				});
		...
		loadList() {
			let scope = this;
			getMethod("/backend/goodAttr/findPage", this.searchParam).then(
				res => {
