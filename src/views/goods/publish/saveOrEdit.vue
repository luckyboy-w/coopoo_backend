      ...
      <el-col :xs="24" :sm="24" :lg="24" class="card-panel-col">
        ...
        <el-form ref="dataForm" :model="dataForm" label-width="100px" label-position="left">
              ...
              <el-table-column align="center" prop="id" label="上传图片">
                <template slot-scope="scope">
                  <el-upload  class="avatar-uploader" :headers="token"
                    :action="uploadSkuImgUrl" :show-file-list="false" :on-success="handleSuccessSkuImg"
          ...
          <el-form-item label="视频">
            <el-input v-show="false" v-model="dataForm.goodsVideo"  />
          ...
          <el-form-item label="封面图片">
            <div id="front-img">
              <el-input v-show="false" v-model="dataForm.goodsCoverImg"  />
              <el-upload :action="uploadGoodsCoverImageUrl" list-type="picture-card" :on-preview="handleGoodImagePreview"
                :before-upload="beforeGoodsCoverImageUpload" :on-success="handleGoodsCoverImageSuccess"
                :class="{hide:hideGoodsCoverImageUpload}" :file-list="uploadGoodsCoverImageList"
          ...
          <el-form-item label="售后说明图片">
            <el-radio-group v-model="dataForm.postSaleId" >
              <el-radio class="my-el-radio" v-for="(item,index) in goodSaleDescList" :key="item.id" :label="item.id">
          ...
          <!-- <el-form-item v-show="false" label="售后保障">
            <qEditor1 ref="refEditor1" :content-q1="detail.postSale" module-name="postSale"
              @changePostSaleContent="changePostSaleContent"  />
          </el-form-item>
          <el-form-item v-show="false" label="商品清单">
            <qEditor2 ref="refEditor2" :q2content="detail.listDetail" module-name="listDetail"
    ...
    methods: {
      ...
      buildGoodImageGroupId() {
        getMethod('/oss/get-group-id', null).then(res => {
          ...
          this.dataForm.goodsImg = this.dataForm.goodsImage || res.data
        })
      },
      ...
      initDefaultImage() {
        ...
        for (let i = 0; i < this.dataForm.files.length; i++) {
          ...
          if (imageObj.groupId == this.dataForm.goodsImg) {
            this.uploadGoodImageList.push(imageObj)
          }
          if (imageObj.groupId == this.dataForm.goodsCoverImg) {
            this.uploadGoodsCoverImageList.push(imageObj)
          }
        }
      ...
      async saveObject() {
        if (this.validate()) {
          ...
          // fileList = fileList.concat(this.uploadSkuImageList)
          ...
          this.dataForm.goodsVideo=this.goodsVideoGroupId
          ...
          try {
            ...
            const {
              data
            } = await postMethod('/goods/publish', param)
            this.typeList = data
            this.detail = {}
