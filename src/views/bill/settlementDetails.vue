          ...
          <el-table ref="mainTable" :data="tableData.list" style="width: 100%; margin-bottom: 20px;" row-key="id" border>
            ...
            <el-table-column prop="goodsName" label="商品名称">
              <template slot-scope="scope">
                {{ scope.row.goodsName }}
                <el-tag v-if="scope.row.isVipOrder == 1" effect="light" style="color: white;background-color: #409EFF;" size="mini">专属</el-tag>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="supplierPreSettlePrice" label="供应商预计结算金额" />
            <el-table-column prop="supplierPlatformPrice" label="供应商平台服务费" />
            <el-table-column prop="supplierSettlePrice" label="供应商实际结算金额" />
...
export default {
  ...
  methods: {
    ...
    exportData() {
      ...
      window.open(process.env.VUE_APP_BASE_API_NEW + '/excel/goods-settlement-detail/export?' + exportParam.join('&'));
    },
    exportStoreData() {
      let exportParam = [];
      ...
      exportParam.push('token=' + getToken());
      console.log(process.env.VUE_APP_BASE_API_NEW + '/excel/supplier-goods-settlement/export?' + exportParam.join('&'));
      window.open(process.env.VUE_APP_BASE_API_NEW + '/excel/supplier-goods-settlement/export?' + exportParam.join('&'));
    },
    loadList() {
      let scope = this;
