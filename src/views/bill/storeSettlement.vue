...
export default {
  ...
  methods: {
    ...
    exportDtl() {
      ...
      window.open(process.env.VUE_APP_BASE_API_NEW + '/bu/orderBill/exportWaitingBillDtl?token=' + getToken() + '&' +
        exportParam.join('&'))
    },
    //待审核导出
    exportData() {
    ...
    exportDataDtl() {
      ...
      console.log(exportParam, '传的参')
      window.open(process.env.VUE_APP_BASE_API_NEW + '/bu/orderBill/exportBillDtl?token=' + getToken() + '&' +
        exportParam.join('&'))
    },
    confirmed(row) {
      let scope = this
