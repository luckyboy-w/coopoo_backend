...
export default {
  ...
  methods: {
    submitReset(){
      ...
      postMethod('/backend/user/resetPwd', this.resetFrm).then(res => {
        this.$message({
					message: "修改完成",
					type: "success"
        });
    ...
    enable(row){
      ...
      postMethod('/backend/user/enable', row).then(res => {
          scope.loadUser()
          this.$message({
            message: "操作成功",
            type: "success"
    ...
    resetPwd(row){
      this.resetFrm.sysUserId = row.sysUserId
      this.showReset = true
    ...
    editSysUser(row){
      ...
      getMethod('/backend/user/findById', param).then(res => {
