	...
	<div class="update-form-panel" v-loading="loading">
		<el-form ref="dataForm" :model="dataForm" label-width="140px">
			...
			<el-form-item label="活动详情">
				<qEditor :content="dataForm.activityContent" :moduleName="moduleName"
	...
	methods: {
		buildActivityImageGroupId() {
			getMethod("/backend/oss/groupId", null).then(res => {
				this.uploadActivityImageUrl = getUploadUrl() + "?groupId=" + (this.dataForm.activityImage || res.data);
		...
		changeContent(val){
			this.dataForm.activityContent = val
		},
		validate() {
