<template>
  <div>
      ...
      <div>
        <div v-if="rowData.activityType == 5">
          <el-table :data="tableData.list" style="width: 100%; margin-bottom: 20px;" row-key="index" border>
            ...
            <el-table-column prop="goodsName" label="商品名称" min-width="170px"></el-table-column>
            ...
            <el-table-column label="操作" fixed="right" width="120px">
              <template slot-scope="scope">
                <el-button @click="getOneCollageRecord(scope.row)" size="mini" type="primary">查看团员</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
    ...
    <el-dialog title="团员详情" :visible.sync="secondTypePopup" width="30%" :append-to-body="true" :close-on-click-modal="false">
      <el-table :data="groupMembersData" row-key="index" border>
        <el-table-column prop="userName" label="团员昵称"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getOneCollageRecord(row) {
      postMethod('/activity/marketing-goods/collage/get-one-collage-record', { collageId: row.collageId }).then(res => {
