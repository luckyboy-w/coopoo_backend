    ...
    <div class="ly-container" v-if="showList">
        <div class="ly-tool-panel" style="display: flex;flex-wrap: wrap;">
          <div class="tabTd">
            <div>广告位置：</div>
            <div>
              <el-select v-model="searchParam.location" clearable>
                <el-option v-for="item in advertLocationList" :key="item.value" :value-key="item.label"
          ...
          <div class="ly-data-list">
            <el-table ref="mainTable" :data="tableData.list" style="width: 100%;margin-bottom: 20px;" row-key="id"
              ...
              <el-table-column prop="id" label="操作" width="200px">
                <template slot-scope="scope">
                  <el-button @click.native.prevent="addOrEdit('edit',scope.$index, tableData)" type="text" size="small">
                    编辑</el-button>

                  <el-button @click="enableAdv(scope.row,0)" type="text" size="small" v-if="scope.row.enable == 1">禁用
                  </el-button>

                  <el-button @click="enableAdv(scope.row,1)" type="text" size="small" v-if="scope.row.enable != 1">启用
                  </el-button>
