<template>
  <div class="bill-tab-list">
    <div v-if="showList" style="margin: 30px;">
      <el-tabs v-model="activeName" @tab-click="handleClick">
        <el-tab-pane label="未结算" name="readyBill">
          ...
          <div class="ly-tool-panel" style="display: flex;flex-wrap: wrap;">
            <div class="tabTd">
              <div>门店名称：</div>
              <div><el-input v-model="searchParam.storeName" width="180px" @keyup.enter.native="search()" /></div>
            </div>
            <div class="tabTd">
          ...
          <el-table ref="noBillData" :data="noBillData.list" style="width: 100%; margin: 20px 0;" row-key="id" border>
        ...
        <el-tab-pane label="待结算" name="settleFinsh">
          ...
          <el-table ref="settleFinshData" :data="settleFinshData.list" style="width: 100%; margin: 20px 0;" row-key="id" border>
            ...
            <el-table-column label="操作" width="200">
              <template slot-scope="scope">
                <el-button size="mini" type="primary" @click="showBillDetail(scope.row)">查看明细</el-button>
                <el-button v-if="scope.row.isConfirm == 1" size="mini" type="primary" @click="confirmed(scope.row)">结算</el-button>
              </template>
            </el-table-column>
          </el-table>
          <el-pagination
        ...
        <el-tab-pane v-if="dataDtl" label="明细" name="dataDtl">
          <span slot="label" class="my-tab-label">明细</span>
          <div class="ly-tool-panel" style="display: flex;flex-wrap: wrap;"><el-button v-if="!back" type="primary" icon="el-icon-back" @click="backTo()">返回列表</el-button></div>
          <div class="ly-tool-panel" style="display: flex;flex-wrap: wrap;">
            <div class="tabTd">
              <div>订单编号：</div>
              <div><el-input v-model="searchParam.orderNo" placeholder="请输入" width="180px" /></div>
            </div>
            <div class="tabTd">
              <div>入账月份：</div>
              <div><el-date-picker v-model="searchParam.accountDate" type="month" value-format="yyyy-MM" placeholder="选择月" /></div>
    ...
    <orderDtl v-if="!showList" ref="orderDtl" :order-no="orderNo" @backToList="backToList" />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    exportDtl() {
      ...
      window.open(process.env.VUE_APP_BASE_API_NEW + '/bu/orderBill/exportWaitingBillDtl?token=' + getToken() + '&' + exportParam.join('&'));
    },
    //待审核导出
    exportData() {
      let exportParam = [];
      ...
      window.open(process.env.VUE_APP_BASE_API_NEW + '/excel/store-settlement/export?' + exportParam.join('&'));
    },
    //已结算导出
    exportData_() {
      if (this.searchParam.startTime == null) {
      ...
      console.log(exportParam, '传的参');
      window.open(process.env.VUE_APP_BASE_API_NEW + '/bu/orderBill/exportBill?token=' + getToken() + '&' + exportParam.join('&'));
    },
    //结算明细导出
    exportDataDtl() {
      if (this.searchParam.startTime == null) {
      ...
      console.log(exportParam, '传的参');
      window.open(process.env.VUE_APP_BASE_API_NEW + '/bu/orderBill/exportBillDtl?token=' + getToken() + '&' + exportParam.join('&'));
    },
    confirmed(row) {
      let scope = this;
